...
def test_web_app_get_form():
    ...
    assert b"Owlet Red Alert Viewer" in response.data
    assert b'type="password"' in response.data
    assert b'name="restrict_24h"' in response.data
    assert b"Restrict to the last 24 hours" in response.data
...
@patch("owlet_api.web_app.decode_histories")
@patch("owlet_api.web_app.download_history")
def test_web_app_can_include_older_samples(mock_download_history, mock_decode_histories):
    mock_download_history.return_value = [{"device_dsn": "abc", "history": []}]
    mock_decode_histories.return_value = [
        {
            "sample_timestamp_utc": "2026-06-08T12:00:00+00:00",
    ...
    assert response.status_code == 200
    assert b"Showing 2 decoded samples." in response.data
    assert b"from the past 24 hours." not in response.data
